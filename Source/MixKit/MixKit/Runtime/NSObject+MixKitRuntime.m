...
@implementation NSObject (mixkit)
...
+ (void)swizzleInitializeOfClassNamed:(NSString *)className {
    ...
    method_exchangeImplementations(class_getClassMethod(class, @selector(initialize)),
                                   class_getClassMethod(self, @selector(_mixkit_initialize)));
}

+ (void)_mixkit_initialize {
    [self _mixkit_initialize];
...
#pragma clang diagnostic ignored "-Wundeclared-selector"
    ...
    SEL performBindingSelector = @selector(_mixkit_performBinding);
