...
/// Represents a response to a `Provider.request`.
...
public extension Response {
    /**
     ...
     - throws: `SwiftyWeiboError.statusCode` when others are encountered.
    */
    public func filter(statusCodes: ClosedRange<Int>) throws -> Response {
        guard statusCodes.contains(statusCode) else {
            throw SwiftyWeiboError.statusCode(self)
        }
        return self
    }
    ...
    */
    ...
    func mapImage() throws -> UIImage {
        guard let image = UIImage(data: data) else {
            throw SwiftyWeiboError.imageMapping(self)
        }
        return image
    }
    ...
    func mapJSON(failsOnEmptyData: Bool = true) throws -> Any {
        do {
            ...
            throw SwiftyWeiboError.jsonMapping(self)
        }
    }
        ...
        if let keyPath = keyPath {
            ...
            guard let jsonDictionary = try mapJSON() as? NSDictionary,
                let string = jsonDictionary.value(forKeyPath:keyPath) as? String else {
                    throw SwiftyWeiboError.stringMapping(self)
            }
            return string
        } else {
