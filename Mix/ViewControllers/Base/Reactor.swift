...
private struct ReactorRuntimeKeys {
    ...
    static let currentStateKey = UnsafeRawPointer(bitPattern: "currentState".hashValue)!
    static let stateKey = UnsafeRawPointer(bitPattern: "state".hashValue)!
    static let disposeBagKey = UnsafeRawPointer(bitPattern: "disposeBag".hashValue)!
    static let stubKey = UnsafeRawPointer(bitPattern: "stub".hashValue)!
}
...
extension Reactor {
    ...
    private var _action: ActionSubject<Action> {
        if self.stub.isEnabled {
            ...
            return self.associatedObject(forKey: ReactorRuntimeKeys.actionKey, default: .init())
        }
    }
    ...
    public internal(set) var currentState: State {
        get { return self.associatedObject(forKey: ReactorRuntimeKeys.currentStateKey, default: self.initialState) }
        set { self.setAssociatedObject(newValue, forKey: ReactorRuntimeKeys.currentStateKey) }
    }
    ...
    public var state: Observable<State> {
        if self.stub.isEnabled {
            ...
            return self.associatedObject(forKey: ReactorRuntimeKeys.stateKey, default: self.createStateStream())
        }
    }
    
    fileprivate var disposeBag: DisposeBag {
        get { return self.associatedObject(forKey: ReactorRuntimeKeys.disposeBagKey, default: DisposeBag()) }
    }
...
extension Reactor {
    public var stub: Stub<Self> {
        return self.associatedObject(
            forKey: ReactorRuntimeKeys.stubKey,
